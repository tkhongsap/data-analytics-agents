...
import { Users, AlertTriangle, Loader } from 'lucide-react';

const UserReportViewer = () => {
  ...
  useEffect(() => {
    const loadReport = async () => {
      try {
        const response = await fetch('/USER_ACCOUNTS_SECURITY_REPORT.md');
        if (!response.ok) {
    ...
    return (
      ...
      >
        <div className="flex items-center space-x-3">
          ...
          <span className="text-gray-300">Loading User Security Report...</span>
        </div>
      </motion.div>
    );
  }
      ...
      <div className="glass-card p-6">
        <div className="flex items-center space-x-3 mb-4">
          ...
          <h2 className="text-2xl font-bold bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent">
            User Accounts Security Report
          </h2>
        </div>
        <p className="text-gray-400">
          Analysis of user account activities, authentication patterns, and security incidents
        </p>
      </div>
      ...
      <div className="glass-card p-8 prose prose-invert max-w-none">
        <ReactMarkdown
          ...
          components={{
            ...
            h3: ({children}) => (
              <h3 className="text-xl font-semibold text-purple-300 mt-6 mb-3">{children}</h3>
            ),
            p: ({children}) => (
            ...
            blockquote: ({children}) => (
              <blockquote className="border-l-4 border-purple-500 pl-4 my-4 text-gray-300 italic">{children}</blockquote>
            ),
            ...
            th: ({children}) => (
              <th className="border border-gray-600 px-4 py-3 text-left font-semibold text-purple-400">{children}</th>
            ),
            td: ({children}) => (
...
export default UserReportViewer;